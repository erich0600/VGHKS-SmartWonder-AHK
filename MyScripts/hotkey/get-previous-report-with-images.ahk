...
GetPreviousReportWithImages(CopyReport=true, LoadImages=true, TotalRecentImages=2, LoadFarImage=true, ShowAlert=true) {
  ...
  wb := WBGet()
  ; frmTabIframe2 := wb.document.parentWindow.frames["tabIframe2"]
  ; MsgBox, % frmTabIframe2.length
  frmWork := wb.document.parentWindow.frames["frameWork"]
  frmTabIframe2 := frmWork.document.parentWindow.frames["tabIframe2"]
  ;MsgBox, % wb.document.getElementsByName["frameWork"].length
 ...
 ;   MsgBox, Not in REPORT EDIT page!
  ...
  test := prevReportLists.children.length
  ...
  If !isNoPrevReport {
    ...
    If (relatedReportCount > 0) {
      ...
      If (LoadImages && relatedReportCount > 1) {
        Loop %TotalRecentImages% {
          If (A_Index > 1 && A_Index <= relatedReportCount) {
            theRelatedReport := prevReportLists.children[prevReportArray[A_Index]].children[3]
      ...
      ; MsgBox, %prevExamDate%
      ; MsgBox, % prevReportLists.children[prevReportArray[1]].children[1]
      ; MsgBox, % prevReportLists.children[prevReportArray[1]].children[3]
      ; copy the latest report

      latestRelatedReport := prevReportLists.children[prevReportArray[1]].children[3]
      latestRelatedReport.click() 
